...
import {
  ...
  AccordionTrigger,
  AccordionContent,
} from "@/components/writings/Accordion";
...
<Accordion type="single" collapsible>
  <AccordionItem value="item-1">
    <AccordionTrigger>Writing Improvements</AccordionTrigger>
    <AccordionContent>
...
<Accordion title="Writing Improvements">
  <ul>
    <li>
      <code>01-writing-simple-improvement</code>
      <ul>
        <li>
    ...
    <li>
      <code>02-writing-distill-and-condense</code> - Asking GPT to remove
      irrelevant information and overall condensing the writing.
    </li>
    <li>
      <code>03-writing-technical-improvement</code> - Asking GPT to apply the
      writing improvements outlined in the{" "}
      <a href="https://writingcenter.unc.edu/tips-and-tools/conciseness-handout">
...
- Writing Improvements - Simplify By Skill Level - `06-simplify-college` - Rewrite
  the passage for someone with a college reading level - `07-simplify-ignorant` - Rewrite
  the passage for someone with no knowledge of the subject matter - `08-simplify-level6`
- Rewrite the passage for someone with a 6th grade reading level - `09-simplify-level12`
- Rewrite the passage for someone with a 12th grade reading level - Role - `10-role-carl-sagan`
- Rewrite the style of Carl Sagan. lol. - `11-role-saul-kahn` - Rewrite the style
  of Saul Kahn. - `12-role-teacher` - Rewrite as if you were a world-renowned teacher
  trying to help a student understand the subject matter. - Transpile - `13-transpile-bulletpoints`
- condense the passage into bullet points - `14-transpile-summary` - summarize the
  passage - Demographic Influence I was initially weary of including these I was hesitant
  to ask the AI to make judgements in rewriting based on age and race… But it’s well
  known that demographics can influence the way people write and speak and I figured
  that if it produced problematic results that be important in exposing the underlying
  biases of the AI. - `04-demographics-author-reader` - Rewrite taking into account
  the demographics (including age, sex, race, education level, socioeconomic status,
  and any other relevant demographic information) of the author and reader - `05-demographics-reader`
- Same as above but for reader (me) only. - Experimental I wasn’t expecting these
  to produce good results (spoiler: they did not) but I was curious to see what they
  would do. - `15-experimental-ai-generated` - I asked Claude3-Sonnet and GPT4 to "Please
  write a prompt for a different LLM AI so that it will rewrite a given paragraph to
  be as easy as possible for someone to understand." and picked the response (Claude3-Sonnet's
  in this case) that I liked more. - `16-experimental-diff-representation` - An experimental
  prompt that attempts to ask the AI to come up with a different way to represent the
  paragraph (e.g., a different language - real or invented, a different style, etc.)
  that it believes would make the paragraph as easy to understand as possible (in that
  new representation).
...
- Rankings per-paragraph if you’re curious
...
<ReactDiffViewer
  ...
  oldValue={`To encourage divergent thinking and foster customization, educators can further generate variant scripts and utilize alternative prompts, expanding the possibilities for tailoring the output content to specific learning objectives.
  ...
  newValue={`To promote divergent thinking and customization, educators can create different scripts and use alternative prompts, broadening the potential to tailor content to specific learning objectives.
...
<ReactDiffViewer
  compareMethod={DiffMethod.WORDS}
  oldValue={`The dispersal of a primordial planetesimal disk drives orbital evolution among the rest of the giant planets as well.
  ...
  newValue={`Imagine a spinning disk of primordial planetesimals, the building blocks of planets. The way this disk disperses influences the orbital evolution of the giant planets in our solar system.
